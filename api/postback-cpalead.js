 ...
 * - Handles CPAlead conversions, credits user points in Supabase DB.
 * - credited_points visada įrašomas į completions, bet balansas pridedamas TIK per Supabase RPC.
 * - Idempotency: pagal user_id + offer_id_partner (leidžia skirtingiems useriams tą patį offerį).
...
export default async function handler(req, res) {
  ...
  } catch (e) { console.error('Failed to log postback:', e) }

  try {
    // Idempotency: tikrina user_id + offer_id_partner
    ...
    // Insert completion (credited_points įrašomas, bet NEPRIDEDA balanso, tiesiog display lentelėje)
    ...
    // Prideda taškus tik per Supabase RPC funkciją, niekur kitur!
