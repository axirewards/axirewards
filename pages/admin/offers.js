              ...
              <tbody>
                {offers.map((offer) => (
                  <tr key={offer.id} className="border-b border-gray-200 dark:border-gray-700">
                    ...
                    <td className="py-2 px-4">
                      <span className={`px-2 py-1 rounded text-white ${offer.status === 'active' ? 'bg-green-600' : 'bg-gray-500'}`}>{offer.status}</span>
                    </td>
                    <td className="py-2 px-4">{new Date(offer.created_at).toLocaleDateString()}</td>
