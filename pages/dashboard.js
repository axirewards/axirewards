    ...
    const getData = async () => {
      try {
        ...
        if (userData.last_login) {
          ...
          setStreak(diffDays === 1 ? (userData.streak || 0) + 1 : 1);
        } else {
          setStreak(1);
        }
        ...
        await supabase
          .from("users")
          .update({ last_login: new Date(), streak })
          .eq("id", userData.id);

        // 5️⃣ Fetch completions with reward info
