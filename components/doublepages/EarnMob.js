...
export default function EarnMob({ setGlobalLoading, router }) {
  ...
  return (
    <Layout>
      <div className="min-h-[80vh] w-full flex flex-col items-center justify-center">
        <div className="w-full max-w-lg py-10 px-2">
          <h1 className="text-3xl font-extrabold text-white text-center mb-8 drop-shadow-xl tracking-tight">
            Earn Rewards
          </h1>
          <div className="flex justify-center mb-8">
            <p className="text-base text-zinc-300 text-center max-w-md font-medium">
          ...
          <div className="grid grid-cols-1 gap-6 w-full">
            {categories.map(cat => (
              <button
                key={cat.key}
                className={`group relative bg-gradient-to-tr ${cat.color} rounded-2xl shadow-2xl flex flex-col items-center justify-center px-6 py-8 transition-all duration-200 
                ...
                aria-label={cat.name}
              >
                <div className={`mb-5 transition-all duration-200 ${hovered === cat.key ? 'scale-110 drop-shadow-2xl' : 'scale-100'}`}>
                  {cat.icon}
                </div>
                <span className="text-xl font-bold text-white drop-shadow mb-2">{cat.name}</span>
                <span className="text-sm text-white/80 text-center px-2">{cat.description}</span>
              </button>
            ))}
          </div>
        </div>
